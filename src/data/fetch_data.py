...
from data.data_constants import LIGHTCURVES_FOLDER_TESS, LIGHTCURVES_FOLDER

# from data_constants import LIGHTCURVES_FOLDER, LIGHTCURVES_FOLDER_TESS
...
#Season 1 Tess
LIGHTCURVES_URL_TESS = "https://archive.stsci.edu/missions/tess/tid/s0001/0000/"
...
#Add leading zeroes to Tess star ids
#############################################################
def get_full_kepid_string(id):
    id_string = str(id)
    id_length = len(id_string)
...
def fetch_fits_file(current_kic_id,file_name,kic_id_link):
        if not os.path.exists(LIGHTCURVES_FOLDER_TESS + '/' + current_kic_id):    
...
def fetch_fits_file_tess(current_kic_id,file_name,kic_id_link):
        if not os.path.exists(LIGHTCURVES_FOLDER_TESS + '/' + current_kic_id):    
            os.mkdir(LIGHTCURVES_FOLDER_TESS + '/'+current_kic_id)
        
        print("Downloading: " + file_name)       
        r = requests.get(kic_id_link + "/" + file_name)
        open(LIGHTCURVES_FOLDER_TESS  + '/' + current_kic_id+"/" + file_name, 'wb').write(r.content) 
...
#############################################################
#current_kic_id : id of the star
def get_kic_id_link_tess(current_kic_id):
    return  LIGHTCURVES_URL_TESS+"000"+current_kic_id[0]+"/"+current_kic_id[1:5]+"/"+current_kic_id[5:9];
...
#############################################################
def download_single_fits_tess(kic_id):
    ...
    #Get the link to the page containing the files for the star
    kic_id_link = get_kic_id_link_tess(current_kic_id)
    ...
    threads = []
    for i in range(0,llc_links_length):
        thread = threading.Thread(target = fetch_fits_file_tess, args=(current_kic_id,llc_links_current_kic[i],kic_id_link,))
        thread.daemon = True
        threads.append(thread)
        print ("Added thread " + str(i))
